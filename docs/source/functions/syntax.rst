...
Do note that when a function is defined without a return type, any ``@return`` statements taking variables will error, as it is probably a mistake.
...
If the function is not in the current namespace, you can either use a different namespace using ``@using``, or using the namespace specifier:
